...
cd /srv/app
sudo chown --recursive ubuntu ./current
...
cd /srv/app/current
/root/.local/bin/aws s3 cp s3://web.devops.stansberryresearch.com/applications/sih/ecosystem.config.js.enc ./
...
# RESTART NGINX
sudo mv /srv/app/current/CodeDeploy/resources/default.conf /etc/nginx/sites-enabled/default
